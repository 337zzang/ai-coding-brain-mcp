...
def test_workflow_progress():
    ...
    from python.workflow.models import Plan, Task, TaskStatus
    ...
    # 검증
    expected_progress = round(2/3 * 100, 1)
    assert completed_count == 2, f"Expected 2 completed tasks, got {completed_count}"
    assert abs(progress - expected_progress) < 0.1, f"Expected ~{expected_progress}% progress, got {progress:.1f}%"
...
def test_helper_result_wrapping():
    ...
    try:
        ...
        from python.helpers_wrapper import HelpersWrapper, safe_helper
