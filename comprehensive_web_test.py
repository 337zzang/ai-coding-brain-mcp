...
class WebAutomationTestSuite:
    ...
    async def test_wait_strategies(self):
        """다양한 대기 전략 테스트"""
        await self.page.goto("https://developer.mozilla.org")

        # 요소 대기
        await self.page.wait_for_selector('.main-header', state='visible')
    ...
    async def test_error_handling(self):
        """에러 처리 테스트"""
        # 404 페이지
        response = await self.page.goto("https://httpstat.us/404")
        assert response.status == 404

        # 타임아웃 페이지
        try:
            await self.page.goto("https://httpstat.us/200?sleep=10000", timeout=2000)
        except:
            pass  # 예상된 타임아웃
