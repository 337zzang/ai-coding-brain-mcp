...
"""
SearchResult 클래스 - o3의 조언을 반영한 구현
"""
...
# safe_search_code 구현
def safe_search_code(base_path: str, pattern: str, file_pattern: str = "*", 
                     case_sensitive: bool = False, max_results: int = 100) -> SearchResult:
    """항상 SearchResult를 반환하는 안전한 버전"""
    try:
        from . import search_code
        results = search_code(base_path, pattern, file_pattern, case_sensitive, max_results)
        return make_search_result(results)
    except Exception as e:
        print(f"검색 중 오류 발생: {e}")
        return SearchResult([])  # 빈 결과 반환
