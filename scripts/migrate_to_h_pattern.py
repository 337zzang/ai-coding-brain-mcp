...
class CodeMigrator:
        ...
        for pattern in ['**/*.py', '**/*.pyw']:
            for filepath in glob.glob(os.path.join(root_dir, pattern), recursive=True):
                ...
                if changes > 0:
                    ...
                    print(f"✅ {os.path.relpath(filepath)}: {changes}개 변경")

        # 결과 요약
        print(f"\n📊 마이그레이션 완료:")
        print(f"  - 수정된 파일: {total_files}개")
        print(f"  - 변경된 함수 호출: {total_changes}개")
        print(f"  - 백업 위치: {self.backup_dir}")

        if self.error_log:
            print(f"\n⚠️ 오류 발생 파일: {len(self.error_log)}개")
    ...
    def generate_report(self, output_file="migration_report.md"):
        ...
        with open(output_file, 'w', encoding='utf-8') as f:
            f.write("# AI Helpers 마이그레이션 리포트\n\n")
            f.write(f"생성일시: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n")

            f.write("## 요약\n")
            f.write(f"- 수정된 파일: {len(self.changes_log)}개\n")
            f.write(f"- 총 변경사항: {sum(log['count'] for log in self.changes_log)}개\n")
            f.write(f"- 백업 폴더: `{self.backup_dir}`\n\n")

            f.write("## 상세 변경 내역\n\n")
            for log in self.changes_log:
                f.write(f"### {log['file']}\n")
                f.write(f"변경 수: {log['count']}개\n\n")

                for change in log['changes'][:5]:  # 처음 5개만 표시
                    f.write(f"**라인 {change['line_num']}**: `{change['function']}` 함수\n")
                    f.write(f"- Before: `{change['old']}`\n")
                    f.write(f"- After: `{change['new']}`\n\n")

                if log['count'] > 5:
                    f.write(f"... 외 {log['count'] - 5}개 변경사항\n\n")

        print(f"\n📄 상세 리포트 생성: {output_file}")
