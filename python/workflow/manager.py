...
class WorkflowManager:
    ...
    def __init__(self, project_name: str, enable_listeners: bool = True):
        """프로젝트별 워크플로우 관리자 초기화"""
        ...
        # Enhanced Listener Integration 자동 활성화
        if enable_listeners:
            try:
                # 전역 EventBus 사용
                ...
                # Enhanced Listener Integration 활성화
                self._register_default_listeners()
                logger.info(f"✅ Listener 시스템 활성화됨 - {self.project_name}")
            except Exception as e:
                logger.error(f"❌ Listener Integration 초기화 실패: {e}")
                self.listener_manager = None
                self.listener_integration = None
    ...
    def _register_default_listeners(self):
        ...
        try:
            # 리스너 모듈 가져오기
            from .listeners.error_collector_listener import ErrorCollectorListener
            from .listeners.docs_generator_listener import DocsGeneratorListener
            from .listeners.task_context_listener import TaskContextListener
            from .listeners.error_listener import ErrorHandlerListener
            from .listeners.workflow_todo_listener import WorkflowTodoListener
            from .listeners.error_to_emergency_todo_listener import ErrorToEmergencyTodoListener
            from .listeners.code_change_report_listener import CodeChangeReportListener
            from .listeners.git_auto_commit_listener import GitAutoCommitListener
            
            # 기본 리스너 등록
            self.listener_manager.register_listener("error_collector", ErrorCollectorListener())
            self.listener_manager.register_listener("docs_generator", DocsGeneratorListener())
            self.listener_manager.register_listener("task_context", TaskContextListener())
            self.listener_manager.register_listener("error_handler", ErrorHandlerListener())
            
            # 새로운 통합 리스너 등록
            self.listener_manager.register_listener("workflow_todo", WorkflowTodoListener())
            self.listener_manager.register_listener("emergency_todo", ErrorToEmergencyTodoListener())
            self.listener_manager.register_listener("code_change_report", CodeChangeReportListener())
            self.listener_manager.register_listener("git_auto_commit", GitAutoCommitListener())
            
            logger.info(f"리스너 8개 등록 완료 (기본 4개 + 통합 4개)")
        except Exception as e:
