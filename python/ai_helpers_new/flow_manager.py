...
class FlowManager:
    ...
    def create_flow(self, name: str = None, project: Optional[str] = None, force: bool = False) -> Flow:
        """Flow 생성 (프로젝트당 하나 원칙)

        Args:
            name: Flow 이름 (기본값: 현재 프로젝트명)
            project: 프로젝트 이름
            force: 강제로 새 Flow 생성 여부
        """
        # 1. Flow 이름 자동 설정 (프로젝트명 사용)
        if not name:
            # 현재 프로젝트명 가져오기
        ...
        if not force:
            existing_flows = self.list_flows()
