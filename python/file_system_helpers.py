...
def replace_block(file_path: str, block_name: str, new_content: str) -> dict:
    ...
    try:
        ...
        try:
            ...
            return {'success': False, 'error': f'구문 오류: {str(e)}', 'details': {'line': e.lineno, 'offset': e.offset, 'text': e.text}}
        ...
        try:
            ...
            try:
                ...
                return {'success': False, 'error': 'ast.unparse는 Python 3.9+ 필요, astor도 설치되지 않음'}
        with open(file_path, 'w', encoding='utf-8') as f:
            f.write(new_content)
        return {'success': True, 'message': f'{block_name} 블록이 성공적으로 교체되었습니다', 'details': {'file': file_path, 'block': block_name, 'original_size': len(original_content), 'new_size': len(new_content), 'ast_based': True}}
    except Exception as e:
        import traceback
...
def insert_block(file_path: str, target_name: str, position: str, new_code: str) -> dict:
    ...
    try:
        ...
        with open(file_path, 'w', encoding='utf-8') as f:
            f.write(new_content)
        return {'success': True, 'message': f'{target_name}의 {position} 위치에 코드가 삽입되었습니다', 'details': {'file': file_path, 'target': target_name, 'position': position, 'original_size': len(original_content), 'new_size': len(new_content), 'ast_based': True}}
    except Exception as e:
        import traceback
        return {'success': False, 'error': str(e), 'traceback': traceback.format_exc()}
