...
from .enhanced_listener_integration import EnhancedListenerIntegration
import logging
...
class WorkflowManager:
    ...
    def __init__(self, project_name: str, enable_listeners: bool = True):
        ...
        if enable_listeners:
            try:
                # EventBus와 ListenerManager 생성
                self.event_bus = EventBus()
                self.listener_manager = ListenerManager(self.event_bus)

                # Enhanced Listener Integration 활성화
                self.listener_integration = EnhancedListenerIntegration(self)
                self.listener_integration.initialize()
                logger.info(f"✅ Enhanced Listener Integration 활성화됨 - {self.project_name}")
