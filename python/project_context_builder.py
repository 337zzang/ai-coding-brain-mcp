...
```
{self._generate_tree_structure(max_depth=3)}
```
"""
...
"""
    ...
    def _generate_tree_structure(self, max_depth: int = 3) -> str:
        ...
        def walk_dir(path: Path, prefix: str = "", depth: int = 0):
            ...
            # 디렉토리만 표시 (파일은 요약)
            for i, dir_item in enumerate(dirs[:8]):  # 최대 8개
                is_last = i == len(dirs) - 1
                tree.append(f"{prefix}{'└── ' if is_last else '├── '}{dir_item.name}/")
            ...
            if len(dirs) > 8:
                tree.append(f"{prefix}└── ... ({len(dirs) - 8} more directories)")
                
            # 중요 파일만 표시
            important_exts = {'.py', '.ts', '.js', '.json', '.md'}
            important_files = [f for f in files if f.suffix in important_exts]
            
            if important_files:
                shown_files = important_files[:3]
                for i, file in enumerate(shown_files):
                    tree.append(f"{prefix}├── {file.name}")
                if len(important_files) > 3:
                    tree.append(f"{prefix}└── ... ({len(important_files) - 3} more files)")
