...
def safe_json_write(file_path, data, encoding='utf-8'):
    ...
    def __init__(self):
        ...
        # LLM operations (llm_ops)
        try:
            from ai_helpers_v2.llm_ops import ask_o3, analyze_code, prepare_o3_context
            
            # 래퍼 함수로 바인딩 (self 문제 회피)
            self.ask_o3 = lambda question, **kwargs: ask_o3(question, **kwargs)
            self.analyze_code = lambda code, question='이 코드를 분석해주세요': analyze_code(code, question)
            self.prepare_o3_context = lambda **kwargs: prepare_o3_context(**kwargs)
            
            # print('✅ LLM operations 로드 성공')  # 조용히 로드
        except ImportError as e:
            # print(f'⚠️ LLM operations 로드 실패: {e}')  # 조용히 실패
            def not_available(*args, **kwargs):
                return {'error': 'LLM operations not available', 'reason': str(e)}
            self.ask_o3 = not_available
            self.analyze_code = not_available
            self.prepare_o3_context = not_available
        except Exception:
            pass  # 다른 오류는 조용히 무시
...
#                             backup_path = current_memory / "workflow_backup.json"  # REMOVED: 불필요한 백업 로직

                            import shutil

#                             shutil.copy2(current_workflow, backup_path)  # REMOVED: 불필요한 백업 로직
